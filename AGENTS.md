...
- **Build a model:** `./build-<name>` (writes `models/<name>/` and runs `ollama create`). Three today: gemma (content), granite (coding/tutor), qwen (general + thinking-on).
...
- **Builder lock-step:** the region below each builder's `Shared assembly` divider is byte-identical across all builders. Change one → change all (or `diff` to verify). Only the top config block (`MODEL_NAME`, `BASE_MODEL`, `ROLE`, `EXTRAS`, `PARAMS`) may differ.
