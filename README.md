...
coding, content/SEO, tool calling, learning, and leak-gated tutoring so the
"which model" decision is measured, not guessed.
...
`qwen3.8-27b-mtp-q4_K_M.gguf` and `gemma4-12b-it-qat.gguf`. Only
`gemma4-12b-it-qat` has its `.mmproj.gguf` vision projector on disk (checked
2026-09-17). Ollama's params, licenses, and ornith's built-in
...
routine comparison of about 1 hour, `deep` for a several-hour confidence run, and
...
`run-tools.py` is the agent test: each task offers a small tool list with
distractors, and scoring is deterministic (right tool, right arguments, nothing
extra, and no call at all where none fits). Five tasks go a round further: the
runner feeds the model's own call a canned result and scores what it does with
it, which is where an agent's real failures live (inventing an address rather
than using the one it looked up, answering from memory after a search, or
claiming success when the tool returned an error). No network: every tool result
is a fixture.
...
**Measured on llama.cpp build 11022, 2026-09-17** (`standard` pass, pinned
offload gemma 22 and qwen 35, 65536 context, `load-mode = none`,
`ubatch-size = 1024`, prompt caching off). Read close rows as ties: the persona suite's
...
the gap between neighbouring models here. The router and kernel logs showed no out-of-memory errors, CUDA errors, GPU
faults, or crashed model processes during the pass. Timing columns (Avg s, prompt tok/s)
...
_Generated by `./eval/promote.py` from 8 runs, 20260917T095456Z–20260917T211246Z (`eval/runs/`). Do not hand-edit: re-run the script._

| Suite | Winner | `lite` | `qwen` | `gemma` |
|---|---|---|---|---|
| Speed | `lite` | 114.3, all GPU, MTP | 55.0, GPU + 35 MoE layers on CPU, MTP | 45.1, GPU + 22 MoE layers on CPU |
| Coding | tie | 24/27 | 26/27 | 25/27 |
| Content | `gemma` | 8/9 | 7/9 | 9/9 |
| Learning | tie | 7.3, 9/12 | 9.8, 12/12 | 9.8, 12/12 |
| Tutor (leak-gated) | `gemma` | 6.9, 4/15 | 5.1, 7/15 | 8.1, 2/15 |
| JSON / long-context | tie | 100%, 2.5 | 100%, 7.3 | 100%, 5.3 |
| Prompt stack | `qwen` | 57% | 100% | 57% |
| Tool calling | `qwen` | 36/39, 92% | 39/39, 100% | 27/39, 69% |
...
- Speed: `eval/runs/20260917T095456Z/speed/summary.md` — 114 tok/s (all GPU, MTP, thinking OFF)
- Coding: `eval/runs/20260917T095551Z/code/summary.md` — 26/27 (96%) @ 73 tok/s
- Content: `eval/runs/20260917T100152Z/content/summary.md` — 9/9 clean (100%) @ 47 tok/s
- Learning: `eval/runs/20260917T101423Z/learn/summary.md` — teach 9.8/10 (code 12/12, explanation 9.8/10)
- Tutor (leak-gated): `eval/runs/20260917T103232Z/tutor/summary.md` — teach 8.1/10 (leaks 2/15, explanation 9.3/10, non-leak explanation 9.4/10)
- JSON / long-context: `eval/runs/20260917T100907Z/json/summary.md` — score 100% (schema 100%, facts 100%)
- Prompt stack: `eval/runs/20260917T100506Z/persona/summary.md` — 21/21 clean (100%)
- Tool calling: `eval/runs/20260917T211246Z/tools/summary.md` — 39/39 (100%)

<!-- BENCH:END -->

### Current picks

From the llama.cpp `standard` pass of 2026-09-17, on build 11022.

| Use | Pick | Basis |
|---|---|---|
| Fast local general use | `lite` | 114 tok/s, the only model fully on GPU. 2.1× `qwen` (55), 2.5× `gemma` (45). |
| Agentic tools (Cline) | `lite` for speed, `qwen` for reliability | Two numbers pull apart. Prompt ingest: `lite` 3021 tok/s vs `gemma` 1545 and `qwen` 1020. Tool calling: `qwen` 39/39, `lite` 36/39, `gemma` 27/39. Pick `lite` for volume, `qwen` when a wrong call costs more than the wait. |
| Tool calling / agent loops | `qwen` | 39/39 vs `lite` 36/39 and `gemma` 27/39. `gemma` skipped the tool on 9 attempts and got an argument wrong on 3. |
| Coding | `gemma` or `qwen` (tie) | `qwen` 26/27, `gemma` 25/27, `lite` 24/27. The top two are inside the tie threshold, and their order flipped from 2026-09-15. |
| Content / SEO / copy | `gemma` | 9/9 clean vs `lite` 8/9 and `qwen` 7/9. `qwen` still runs over word limits. |
| Socratic tutoring (no spoilers) | `gemma` | Fewest leaks: 2/15 vs `lite` 4/15 and `qwen` 7/15. Teach 8.1/10 vs 6.9 and 5.1. |
| Learning explanations | `qwen` or `gemma` (tie) | Both 9.8/10 with 12/12 on the code gate. |
| Structured JSON / app smoke tests | `lite` | All three scored 100%. `lite` averages 2.5s against `gemma` 5.3s and `qwen` 7.3s. |
| Prompt-stack fidelity | `qwen` | 100% clean vs 57% for both others. If the stack's rules have to be obeyed, this is the model that obeys them. |

No single model wins. `gemma` takes content and tutoring, `lite` takes everything
speed-shaped, `qwen` takes the prompt stack and tool calling, and coding and
learning are ties between `gemma` and `qwen`. The tool suite is the one place
`gemma` is clearly weakest, which matters if you point an agent at it.
